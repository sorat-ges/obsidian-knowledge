...
### Input จาก Remarketer Webhook (จริง)
...
```
┌─────────────────────────────────────────────────────────────────┐
│ Step 1: ดึงข้อมูลจาก Webhook                                   │
├─────────────────────────────────────────────────────────────────┤
│ order_side = "SELL"                                              │
│ received_quantity = 199.50 THB    ← เงินที่ได้รับ (หัก exchange_fee แล้ว) │
│ exchange_fee = 0.50 THB           ← ค่าธรรมเนียมจาก Exchange         │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│ Step 2: บวก Exchange Fee กลับเข้าไป (ReceivedQuantity)       │
├─────────────────────────────────────────────────────────────────┤
│ adjustedReceived = 199.50 + 0.50 = 200.00 THB                   │
│                                                                  │
│ ※ ทำไมต้องบวกกลับ?                                           │
│   - Webhook ส่ง received_quantity ที่หัก exchange_fee ออกแล้ว      │
│   - แต่เราต้องคำนวณ OrderFee จากยอดเต็ม (ก่อนหัก exchange_fee) │
│   - จึงต้องบวก exchange_fee กลับเข้าไปก่อนคำนวณ               │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│ Step 3: คำนวณ OrderFee                                          │
├─────────────────────────────────────────────────────────────────┤
│ orderFee = adjustedReceived × (swapFee / 100)                   │
│ orderFee = 200.00 × (0.12 / 100)                                │
│ orderFee = 200.00 × 0.0012                                      │
│ orderFee = 0.24 THB                                             │
│                                                                  │
│ RoundDown(0.24, 2) = 0.24 THB                                    │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
...
│ vat = 1.68 / 107                                                 │
│ vat = 0.0157...                                                  │
│ vat = Round(0.0157, 2) = 0.02 THB                                │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│ Step 5: คำนวณ NetReceivedQuantity (เงินที่ลูกค้าได้รับสุทธิ)    │
├─────────────────────────────────────────────────────────────────┤
│ ※ โค้ดจริง: NetReceivedQuantity = ReceivedQuantity() - OrderFee │
│                                                                  │
│ Where:                                                           │
│   ReceivedQuantity() = received_quantity + exchange_fee         │
│                      = 199.50 + 0.50 = 200.00 THB                │
│   OrderFee = 0.24 THB                                            │
│                                                                  │
│ netReceived = ReceivedQuantity() - OrderFee                     │
│ netReceived = 200.00 - 0.24                                      │
│ netReceived = 199.76 THB                                         │
│                                                                  │
│ ※ สังเกต: โค้ดหัก OrderFee จากยอดที่บวก exchange_fee แล้ว         │
│           (ไม่ได้หักจาก received_quantity ต้นทาง 199.50)           │
└─────────────────────────────────────────────────────────────────┘
```
...
|---------|-----------|----------|
| **received_quantity** (จาก Webhook) | 199.50 THB | เงินที่ได้จากการขาย BTC (หัก exchange_fee แล้ว) |
| **exchange_fee** | 0.50 THB | ค่าธรรมเนียม Exchange |
| **ReceivedQuantity()** (บวก exchange_fee กลับ) | 200.00 THB | ยอดเต็มก่อนหัก exchange_fee ใช้สำหรับคำนวณ OrderFee |
| **OrderFee (คำนวณได้)** | **0.24 THB** | ค่าธรรมเนียม XAS (0.12% ของ 200.00) |
| **VAT** | 0.02 THB | ภาษีมูลค่าเพิ่ม |
| **net_received_quantity** | 199.76 THB | เงินที่ลูกค้าได้รับสุทธิ (200.00 - 0.24) |

### คำถาม: OrderFee หักจากไหน?
...
Webhook: received_quantity = 199.50 THB
                ↓
ReceivedQuantity(): 199.50 + 0.50 = 200.00 THB  ← บวก exchange_fee กลับ
                ↓
OrderFee = 200.00 × 0.0012 = 0.24 THB
                ↓
NetReceivedQuantity = 200.00 - 0.24 = 199.76 THB  ← หักจาก 200.00
```

**สรุป:**
1. OrderFee คำนวณจากยอดเต็ม 200.00 THB → ได้ 0.24 THB
2. NetReceivedQuantity หัก OrderFee ออกจาก **ReceivedQuantity() (200.00)**
3. เงินที่ลูกค้าได้รับสุทธิ = **200.00 - 0.24 = 199.76 THB**

⚠️ **สังเกต:** โค้ดไม่ได้หัก OrderFee จาก received_quantity (199.50) โดยตรง
- แต่บวก exchange_fee กลับเข้าไปก่อน → 200.00
- แล้วค่อยหัก OrderFee ออก → 199.76
...
├─────────────────────────────────────────────────────────────────┤
│ order_side = "BUY"                                               │
│ executed_quantity = 10,000.00 THB   ← เงินที่ลูกค้าจ่ายไป     │
│ received_quantity = 0.00492500 BTC  ← BTC ที่ได้รับ             │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│ Step 2: คำนวณ OrderFee (สำหรับ BUY ไม่ต้องบวก exchange_fee)   │
├─────────────────────────────────────────────────────────────────┤
│ orderFee = executedQuantity × (swapFee / 100)                   │
│ orderFee = 10,000.00 × (0.12 / 100)                             │
│ orderFee = 10,000.00 × 0.0012                                   │
│ orderFee = 12.00 THB                                             │
│                                                                  │
│ RoundDown(12.00, 2) = 12.00 THB                                  │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
...
│ vat = 84.00 / 107                                                │
│ vat = 0.7850...                                                  │
│ vat = Round(0.7850, 2) = 0.79 THB                                │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│ Step 4: คำนวณ NetReceivedQuantity (BTC ที่ลูกค้าได้รับสุทธิ)    │
├─────────────────────────────────────────────────────────────────┤
│ netReceivedBTC = received_quantity                              │
│               = 0.00492500 BTC                                   │
│                                                                  │
│ (สำหรับ BUY, OrderFee หักจาก executed_quantity/THB              │
│  ไม่ได้หักจาก received_quantity/BTC)                            │
└─────────────────────────────────────────────────────────────────┘
```
...
| รายการ | จำนวนเงิน | หมายเหตุ |
|---------|-----------|----------|
| **executed_quantity** (จาก Webhook) | 10,000.00 THB | เงินที่ลูกค้าจ่าย |
| **received_quantity** | 0.00492500 BTC | BTC ที่ได้รับ |
| **OrderFee (คำนวณได้)** | **12.00 THB** | ค่าธรรมเนียม XAS (0.12% ของ 10,000) |
| **VAT** | 0.79 THB | ภาษีมูลค่าเพิ่ม |
| **net_received_quantity** | 0.00492500 BTC | BTC ที่ลูกค้าได้รับสุทธิ |

### ความแตกต่างระหว่าง BUY และ SELL

| ด้าน | ฐานในการคำนวณ OrderFee | OrderFee หักจาก |
|------|-------------------------|------------------|
| **BUY** | executed_quantity (THB) | หักจาก THB ที่จ่าย |
| **SELL** | adjustedReceived (THB + exchange_fee) | หักจาก received_quantity (THB ที่ได้รับ) |

---

## 5.6 Edge Case: เมื่อ OrderFee > exchange_fee

### ปัญหา

จากโค้ด `filled_swap.go:133-178` สำหรับ SELL:
- `ReceivedQuantity()` บวก `exchange_fee` กลับเข้าไป → `199.50 + 0.50 = 200.00`
- `OrderFee` คำนวณจาก `ReceivedQuantity()` → `200.00 × 0.12% = 0.24`
- `NetReceivedQuantity` = `ReceivedQuantity() - OrderFee` → `200.00 - 0.24 = 199.76`

**แต่ถ้า OrderFee > exchange_fee จะเกิดอะไรขึ้น?**

### ตัวอย่างกรณี OrderFee สูง

```
Input:
- received_quantity = 199.50 THB
- exchange_fee = 0.10 THB (ต่ำ)
- swap_fee_rate = 0.50% (สูง)

คำนวณ:
1. ReceivedQuantity() = 199.50 + 0.10 = 199.60 THB
2. OrderFee = 199.60 × 0.005 = 0.99 THB ← OrderFee > exchange_fee!
3. NetReceivedQuantity = 199.60 - 0.99 = 198.61 THB

ผลลัพธ์:
- ลูกค้าได้จาก Exchange: 199.50 THB
- ลูกค้าได้สุทธิ: 198.61 THB
- เสียประโยชน์: -0.89 THB ⚠️
```

### เปรียบเทียบกรณี

| Case | received_qty | exchange_fee | fee_rate | OrderFee | NetReceived | ผล |
|------|--------------|--------------|----------|----------|-------------|-----|
| **ปกติ** | 199.50 | 0.50 | 0.12% | 0.24 | 199.76 | ✅ ได้มากกว่า |
| **OrderFee > ex.fee** | 199.50 | 0.10 | 0.50% | 0.99 | 198.61 | ⚠️ เสียเปรียบ |
| **วิกฤต** | 100.00 | 0.50 | 5.00% | 5.02 | 95.48 | 🔴 เสียเยอะ |

### โค้ดมี validation ไหม?

**ไฟล์:** `pkg/order_trade/webhook_service.go:497-555`

```go
func (s *orderTradeService) validateFilledSwap(filledSwap domain.FilledSwap) {
    // ✓ เช็คแค่ parsing ข้อมูล:
    matchDate, executedPrice, executedQuantity, receivedQuantity, exchangeFee

    // ✗ ไม่มีเช็ค business logic:
    // - OrderFee เทียบกับ exchange_fee?
    // - NetReceivedQuantity >= received_quantity?
}
```

**สรุป:** โค้ดตอนนี้ **ไม่มี validation** สำหรับกรณีนี้ → จะคำนวณได้ปกติแม้ OrderFee > exchange_fee
...
หลังจากคำนวณ OrderFee และ VAT เสร็จแล้ว ข้อมูลจะถูกบันทึกลง Database และดำเนินการต่อไป

```
┌─────────────────────────────────────────────────────────────────┐
│         insertOrderTransactionAndExchange (Line 391-495)         │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  1. GetPossibleFeeRate                                          │
│     └── ดึง Fee Rate จาก transaction_fee table                   │
│                                                                  │
│  2. CalculateTotalFeeRate                                       │
│     └── totalSwapFee = sum(feeRate[].FeeValue)                  │
│                                                                  │
...
│     └── สร้าง FilledSwap พร้อม SwapFee                          │
│                                                                  │
│  4. validateFilledSwap                                          │
│     └── ตรวจสอบความถูกต้องของข้อมูล                         │
│                                                                  │
│  5. Marshal feeRateResult → JSON                                │
│     └── เตรียมข้อมูล FeeDetail สำหรับเก็บใน DB               │
│                                                                  │
│  6. สร้าง OrderTradeTransaction                                 │
│     ├── OrderFee ← filledSwap.OrderFee()                        │
│     ├── Vat ← filledSwap.Vat()                                  │
│     ├── OrderFeeRate ← totalSwapFee                             │
│     ├── FeeDetail ← JSON of feeRateResult                       │
│     └── NetReceivedQuantity ← คำนวณจาก filledSwap                │
│                                                                  │
│  7. Insert into order_trade_transaction table                    │
│                                                                  │
│  8. getOrderTradeExchange                                       │
│     └── สร้าง OrderTradeExchange records                         │
│                                                                  │
│  9. Insert into order_trade_exchange table                       │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│      calculateAndUpdateOrderTradeInformation (Line 996-1052)     │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  1. ดึง OrderTradeTransaction ทั้งหมดของ OrderTrade นี้          │
│  2. ดึง OrderTradeExchange ทั้งหมด                              │
│  3. Calculate Aggregates:                                        │
│     ├── SumOrderFee                                              │
│     ├── SumVat                                                   │
│     ├── SumNetReceivedQuantity                                   │
│     ├── AvgFeeRate                                               │
│     └── อื่นๆ ...                                               │
│  4. Update order_trade_information table                         │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│            handleOnCompleteOrderTrade (Line 614-758)             │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  1. Update ActionFlow (FILLED → SYNC_LEDGER)                     │
│  2. Update OrderTrade Status → SYNC_LEDGER                       │
│  3. Create Ledger Transactions:                                  │
│     ├── Physical Ledger (Physical Ledger Transaction)            │
│     ├── Logical Ledger (Logical Ledger Transaction)              │
│     └── Send to Ledger Service via Kafka                         │
│  4. Update ActionFlow (SYNC_LEDGER → SYNCED)                     │
│  5. Update OrderTrade Status → FILLED                            │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

## 11.2 รายละเอียดการบันทึก OrderTradeTransaction

**ไฟล์:** `pkg/order_trade/webhook_service.go` (บรรทัด 431-482)

### ฟิลด์ที่เกี่ยวข้องกับ Fee

| Field | Type | Source | Description |
|-------|------|--------|-------------|
| `order_fee` | decimal | `filledSwap.OrderFee()` | ค่าธรรมเนียมที่คำนวณได้ |
| `vat` | decimal | `filledSwap.Vat()` | ภาษีมูลค่าเพิ่มที่คำนวณได้ |
| `order_fee_rate` | decimal | `totalSwapFee` | Fee Rate รวม (%) |
| `fee_detail` | JSON | `feeRateResult` | รายละเอียด Fee ทั้งหมดแบบ JSON |
| `net_received_quantity` | decimal | `filledSwap.NetReceivedQuantity()` | จำนวนที่ได้รับสุทธิ |

### โครงสร้าง fee_detail (JSON)
...
## 11.3 การอัปเดต OrderTradeInformation

**ไฟล์:** `pkg/order_trade/webhook_service.go` (บรรทัด 996-1052)

หลังจากบันทึก OrderTradeTransaction แล้ว จะมีการรวมข้อมูลทั้งหมดของ OrderTrade เพื่ออัปเดต order_trade_information

### Aggregates ที่คำนวณ

| Field | Formula | Description |
|-------|---------|-------------|
| `sum_order_fee` | `sum(tx.order_fee)` | รวม OrderFee ทุก Transaction |
| `sum_vat` | `sum(tx.vat)` | รวม VAT ทุก Transaction |
| `sum_net_received_quantity` | `sum(tx.net_received_quantity)` | รวมจำนวนที่ได้รับสุทธิ |
| `avg_fee_rate` | `sum_order_fee / total_fee_quantity` | อัตรา Fee เฉลี่ย |

### ตัวอย่างคำนวณ avg_fee_rate

```go
// จากไฟล์ calculateOrderTradeTransactionAggregates (บรรทัด 1105-1221)
if orderSide == BUY {
    totalFeeQuantity = sumExecutedQuantity  // ใช้ executed quantity
} else if orderSide == SELL {
    totalFeeQuantity = sumReceivedQuantity  // ใช้ received quantity
}
...
## 11.4 การส่งข้อมูลไป Ledger Service

หลังจากอัปเดต OrderTradeInformation แล้ว ข้อมูลจะถูกส่งไปยัง Ledger Service เพื่อบันทึกบัญชี

### Flow การสร้าง Ledger Transaction

**ไฟล์:** `pkg/order_trade/webhook_service.go` (บรรทัด 669-710)

```
┌─────────────────────────────────────────────────────────────────┐
│                  getCreatedLedgerTransactions                    │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  BUY Order:                                                      │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │ 1. stampPhysicalLedgerTransactionSwapBuyFilled           │   │
│  │    └── Create Physical Ledger Transaction                │   │
│  │                                                          │   │
│  │ 2. stampExecutedLogicalLedgerTransactionSwapBuy          │   │
│  │    └── Create Logical Ledger Transaction (Executed)      │   │
│  │                                                          │   │
│  │ 3. stampRefundedLogicalLedgerTransactionSwapBuy          │   │
│  │    └── Create Logical Ledger Transaction (Refunded)      │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                  │
│  SELL Order:                                                     │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │ 1. stampPhysicalLedgerTransactionSwapSellFilled          │   │
│  │    └── Create Physical Ledger Transaction                │   │
│  │                                                          │   │
│  │ 2. stampExecutedLogicalLedgerTransactionSwapSell         │   │
│  │    └── Create Logical Ledger Transaction (Executed)      │   │
│  │                                                          │   │
│  │ 3. stampRefundedLogicalLedgerTransactionSwapSell         │   │
│  │    └── Create Logical Ledger Transaction (Refunded)      │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                  │
│  4. produceCustomerTransactionLogical                            │
│     └── Send to Kafka Topic (Customer Transaction Logical)      │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
...
### ข้อมูลที่ส่งไป Ledger Service

ข้อมูลที่ส่งไปประกอบด้วย:
- **OrderTrade** - ข้อมูล Order
- **OrderTradeTransaction** - รวม OrderFee, Vat
- **OrderTradeExchange** - ข้อมูลการแลกเปลี่ยน
- **LogicalLedgerTransaction** - รายการบัญชีบัญชี

## 11.5 ตัวอย่างข้อมูลที่บันทึกใน Database

### order_trade_transaction table

```
┌──────────────────────────────┬───────────────┬──────────┬──────────┬──────────────┬─────────────────┐
│ id                           │ order_fee     │ vat      │ fee_rate │ fee_detail   │ net_received_qty │
├──────────────────────────────┼───────────────┼──────────┼──────────┼──────────────┼─────────────────┤
│ tx-001                       │ 12.00         │ 0.78     │ 0.12     | [...]        │ 9987.22         │
│                              │ THB           │ THB      │ %        │              │ THB             │
├──────────────────────────────┼───────────────┼──────────┼──────────┼──────────────┼─────────────────┤
│ tx-002 (partial fill)        │ 6.00          │ 0.39     │ 0.12     │ [...]        │ 4993.61         │
│                              │ THB           │ THB      │ %        │              │ THB             │
└──────────────────────────────┴───────────────┴──────────┴──────────┴──────────────┴─────────────────┘
```
...
```
┌──────────────────────────────┬────────────────┬──────────┬───────────────────┬───────────────┐
│ id                           │ sum_order_fee  │ sum_vat  │ avg_fee_rate      │ sum_net_qty   │
├──────────────────────────────┼────────────────┼──────────┼───────────────────┼───────────────┤
│ info-001                     │ 18.00 THB      │ 1.17 THB │ 0.0012            │ 14980.83 THB  │
│                              │                │          │ (0.12%)           │               │
└──────────────────────────────┴────────────────┴──────────┴───────────────────┴───────────────┘
```

## 11.6 สรุป Flow หลังคำนวณ OrderFee

```
Step 1: คำนวณ OrderFee & VAT
   ↓
Step 2: เตรียม FeeDetail (JSON)
   ↓
Step 3: บันทึกลง order_trade_transaction
   ├── order_fee
   ├── vat
   ├── order_fee_rate
   ├── fee_detail (JSON)
   ...
   ↓
Step 4: บันทึก order_trade_exchange
   ↓
Step 5: รวม Aggregates และอัปเดต order_trade_information
   ├── sum_order_fee
   ├── sum_vat
   ├── avg_fee_rate
   └── sum_net_received_quantity
   ↓
Step 6: สร้าง Ledger Transactions
   ├── Physical Ledger
   └── Logical Ledger (ส่งไป Ledger Service)
   ↓
Step 7: อัปเดต OrderTrade Status → FILLED
```
...
| **บันทึก OrderFee**    | Table `order_trade_transaction.order_fee`         |
| **บันทึก VAT**         | Table `order_trade_transaction.vat`               |
